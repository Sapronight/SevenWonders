...
class City {
  ...
  init() {
    this.divinity_.init();
    this.divinity_.worldEvents.on('favor', ressource =>
      this.getCityCG(ressource)
    );
    this.divinity_.worldEvents.on('blessing', ressource =>
      this.getCityCG(ressource)
    );
  }
    ...
    switch (name) {
      case 'Dragon':
        if (this.gold_ >= 500 && this.maxArmy_ > 0) {
          ...
          console.log(`La cité: ${
            this.name_
          } ne peut pas invoquer : Dragon, car G : ${this.gold_} 
      ...
      case 'Peon':
        if (this.gold_ >= 20 && this.maxArmy_ > 0) {
          ...
          console.log(`La cité: ${
            this.name_
          } ne peut pas invoquer : Peon, car G : ${this.gold_} 
					< 500 ou maxArmy: ${this.maxArmy_} <= 0`);
        }
      ...
      case 'Berserker':
        if (this.gold_ >= 50 && this.maxArmy_ > 0) {
          ...
          console.log(`La cité: ${
            this.name_
          } ne peut pas invoquer : Berserker, car G : ${this.gold_} 
					< 500 ou maxArmy: ${this.maxArmy_} <= 0`);
        }
      ...
      case 'Shield':
        if (this.gold_ >= 200 && this.maxArmy_ > 0) {
          ...
          console.log(`La cité: ${
            this.name_
          } ne peut pas invoquer : Shield, car G : ${this.gold_} 
					< 500 ou maxArmy: ${this.maxArmy_} <= 0`);
        }
  ...
  actionArmy(enemy) {
    for (let i = 0; i < this.tableArmy_.length; i++) {
      if (
        this.tableArmy_[i].mode_ === 'combat' &&
        this.tableArmy_[i].status_ === 'healthy'
      ) {
        this.tableArmy_[i].combat(enemy);
      } else if (
        this.tableArmy_[i].mode_ === 'combat' &&
        this.tableArmy_[i].status_ === 'injure'
      ) {
  ...
  checkArmy() {
    ...
    for (let i = 0; i < this.tableArmy_.length; i++) {
      if (this.tableArmy_[i].status_ === 'dead') {
        tabTemp.push(i);
        console.log(
          `L'unité: ${this.tableArmy_[i].name_} de la ville: ${
            this.name_
          } est morte !`
        );
